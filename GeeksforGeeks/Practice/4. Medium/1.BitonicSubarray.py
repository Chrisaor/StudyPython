def bitnoic_subarr(arr):
    ...
    for i in range(len(arr)):
        ...
        if len(temp) == 0:
            ...
            temp.append(arr[i])
            print('append next element', temp)
            if temp[-1] < temp[-2]:
                des = 1
                print('descending')
            elif temp[-1] > temp[-2]:
            ...
            if temp[-1] > arr[i]:  # 낮은 원소 발견
                ...
                print('now descending', temp) # 내림차순으로 진행중
            ...
            if temp[-1] < arr[i]:
                ...
                print('descending result :', len(temp), i, 'at element')
                asc = 0
                des = 0
                temp = temp[-1:]
